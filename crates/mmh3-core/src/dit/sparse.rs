//! Sol-Attn, a training-free block-sparse attention for video diffusion (arXiv 2607.24027): settings for the DiT
//! and a CPU reference of the algorithm.
//!
//! Tokens form blocks of 64. Each query block attends a routed subset of key blocks token by token. Every other key
//! block contributes one pooled term, its mean key scored against the query block's mean query, weighted by its
//! length and carrying its summed values, so the softmax still covers the whole sequence. A key block is routed when
//! its pooled score clears tau standard deviations of the query block's pooled score distribution, and the diagonal
//! blocks, the conditioning prefix and the dense query rows are always routed.

use crate::dit::layout::{PackedLayout, SegmentKind};

pub const SPARSE_BLOCK: usize = 64;

/// When and how sparsely the DiT uses Sol-Attn. The defaults follow ComfyUI's Model Sparse Attention node.
...
impl Default for SparseAttention {
    fn default() -> Self {
        SparseAttention { tau: 1.3, start_fraction: 0.2, min_tokens: 12_288 }
    }
}
...
impl SparseSinks {
    /// H3's conditioning: every query attends the text and audio prefix exactly, and the audio queries run dense.
    pub fn for_layout(layout: &PackedLayout) -> Self {
        ...
        SparseSinks { key_blocks: (0, prefix_blocks), query_blocks: (audio.start / SPARSE_BLOCK, prefix_blocks) }
    }
}

/// Sol-Attn over `[tokens, heads, dim]` query, key and value, computed densely in f64. Returns the output in the same
/// layout and the fraction of (head, query block, key block) triples routed exactly.
...
pub fn reference(
    ...
    let at = |tensor: &[f32], token: usize, head: usize, index: usize| tensor[(token * heads + head) * dim + index] as f64;
    ...
    for head in 0..heads {
        ...
        let pool = |tensor: &[f32], mean: bool| -> Vec<Vec<f64>> {
            (0..blocks)
                .map(|block| {
                    let rows = block * SPARSE_BLOCK..(block * SPARSE_BLOCK + SPARSE_BLOCK).min(tokens);
                    let divisor = if mean { length(block) } else { 1.0 };
                    (0..dim).map(|index| rows.clone().map(|token| at(tensor, token, head, index)).sum::<f64>() / divisor).collect()
                })
                .collect()
        };
        ...
        let key_mean: Vec<f64> = (0..dim).map(|index| block_keys.iter().map(|row| row[index]).sum::<f64>() / blocks as f64).collect();
        for row in &mut block_keys {
            for (value, mean) in row.iter_mut().zip(&key_mean) {
                *value -= mean;
            }
        }
        let key_variance: Vec<f64> = (0..dim).map(|index| block_keys.iter().map(|row| row[index] * row[index]).sum::<f64>() / blocks as f64).collect();
        ...
        for query_block in 0..blocks {
            ...
            let spread: f64 = centroid.iter().zip(&key_variance).map(|(c, variance)| c * c * variance).sum();
            let threshold = tau as f64 * (spread * log2_scale * log2_scale + 1e-6).sqrt();
            let pooled: Vec<f64> = block_keys.iter().map(|block_key| dot(centroid, block_key) * log2_scale).collect();
            ...
            for token in query_block * SPARSE_BLOCK..(query_block * SPARSE_BLOCK + SPARSE_BLOCK).min(tokens) {
                let row: Vec<f64> = (0..dim).map(|index| at(query, token, head, index)).collect();
                let mut terms: Vec<(f64, f64, Vec<f64>)> = Vec::new();
                for key_block in 0..blocks {
                    if exact[key_block] {
                        for other in key_block * SPARSE_BLOCK..(key_block * SPARSE_BLOCK + SPARSE_BLOCK).min(tokens) {
                            let centered: Vec<f64> = (0..dim).map(|index| at(key, other, head, index) - key_mean[index]).collect();
                            let values = (0..dim).map(|index| at(value, other, head, index)).collect();
                            terms.push((dot(&row, &centered) * log2_scale, 1.0, values));
                        }
                    } else {
                        terms.push((pooled[key_block], length(key_block), value_sums[key_block].clone()));
                    }
                }
                ...
                for index in 0..dim {
                    output[(token * heads + head) * dim + index] = (numerator[index] / denominator) as f32;
                }
            }
        }
    }
...
mod tests {
    ...
    fn routes_everything_like_dense_attention() {
        ...
        let values: Vec<f32> = (0..tokens * heads * dim).map(|index| ((index * 37 % 17) as f32 - 8.0) / 8.0).collect();
        let sinks = SparseSinks { key_blocks: (0, 2), query_blocks: (0, 0) };
        let (output, routed) = reference(&values, &values, &values, tokens, heads, dim, 0.0, 0.5, sinks);
        assert_eq!(routed, 1.0);
        for token in [0, 33, 69] {
            let scores: Vec<f64> = (0..tokens)
                .map(|other| (0..dim).map(|index| values[token * dim + index] as f64 * values[other * dim + index] as f64).sum::<f64>() * 0.5)
                .collect();
            ...
            for index in 0..dim {
                let expected: f64 = (0..tokens).map(|other| weights[other] * values[other * dim + index] as f64).sum::<f64>() / total;
    ...
    fn places_the_h3_sinks_on_the_conditioning() {
        ...
        assert_eq!(sinks, SparseSinks { key_blocks: (0, 3), query_blocks: (1, 3) });
