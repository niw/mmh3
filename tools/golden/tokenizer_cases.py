"""Writes tokenizer test cases: texts and their ids from Hugging Face tokenizers.

Development-only tool. It needs the Hugging Face tokenizers package, which ComfyUI
installs, and the tokenizer.json of MiniMaxAI/MiniMax-H3, for example:

    python3 tools/golden/tokenizer_cases.py \\
        --tokenizer /path/to/MiniMax-H3/tokenizer/tokenizer.json \\
        --out tests/fixtures/tokenizer_cases.json

The tokenizer gets the same extra special tokens as ComfyUI's MiniMax H3 tokenizer, and
nothing is added around the text, as H3 conditions on the raw prompt.
"""
...
def main():
    ...
    assert [tokenizer.token_to_id(token) for token in EXTRA_TOKENS] == list(
        range(151669, 151676)
    )
    cases = [
