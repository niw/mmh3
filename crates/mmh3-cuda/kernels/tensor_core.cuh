...
// Tensor core building blocks shared by the attention kernels: cp.async copies, ldmatrix loads, the m16n8k16 MMA
// with FP32 accumulation, and 64-row tiles of K or V swizzled in shared memory.

// Element (batch, token, head, dimension) of the query, key, value and output tensors, in that order, lives at
// base + batch * batch_stride + token * token_stride + head * head_stride + dimension.
...
__device__ __forceinline__ void load_matrix_x4_transposed(uint32_t (&registers)[4], uint32_t address) {
...
struct Numeric<__nv_bfloat16> {
    __device__ static void mma(float (&accumulator)[4], const uint32_t (&a)[4], uint32_t b0, uint32_t b1) {
        asm volatile(
...
struct Tiles {
    ...
    // XOR-swizzling the chunk by (row % 8) makes every ldmatrix phase, plain or transposed, touch eight distinct
    // 16-byte bank groups.
...
__device__ __forceinline__ void load_rows(uint32_t destination, const Element* base, int64_t stride, int first_row,
    ...
    for (int index = threadIdx.x; index < row_count * T::chunks_per_row; index += THREAD_COUNT) {
        ...
        const Element* source = base + (valid ? static_cast<int64_t>(first_row + row) * stride : 0) + chunk * 8;
