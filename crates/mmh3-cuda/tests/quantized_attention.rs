...
fn bounded_error_with_ragged_tiles_and_reused_scales() {
    ...
    for tokens in [1usize, 63, 64, 65, 127, 129, 200] {
        ...
        for pass in 0..3 {
            let values: Vec<u8> = (0..tokens * 3 * inner)
                .flat_map(|index| {
                    random = random
                    ...
                    let factor = if (index / inner) % 3 == 2 {
                        2f32.powi((head as i32 - 1) * (pass + 1) * 3)
                    } else {
                        1.0
                    };
            ...
            if pass == 2 {
                assert!(
                    ...
                    "zero input must clear prior results"
                );
                continue;
            }
            for head in 0..heads {
                ...
                assert!(
                    relative < 0.05,
                    "tokens {tokens}, head {head}, pass {pass}: relative L2 {relative}"
                );
            }
        }
    }
...
fn zero_queries_produce_uniform_means_including_the_last_key() {
    ...
    for tokens in [1usize, 63, 64, 65, 129] {
        ...
        for token in 0..tokens {
            for dimension in 0..HEAD_DIM {
                qkv[(token * 3 + 2) * HEAD_DIM + dimension] =
                    f32_to_bf16(if (token + dimension) % 2 == 0 {
                        448.0
                    } else {
                        -448.0
                    });
        ...
        input
            .copy_from_host(&qkv.iter().flat_map(|v| v.to_le_bytes()).collect::<Vec<_>>())
            .unwrap();
