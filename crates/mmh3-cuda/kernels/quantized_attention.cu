...
__global__ void value_max(const __nv_bfloat16* value, int tokens, Mmh3AttentionLayout layout, float* partial) {
    ...
    for (int index = threadIdx.x; index < 64 * HEAD_DIM; index += 256) {
        ...
        if (token < tokens) {
            maximum = fmaxf(maximum, fabsf(__bfloat162float(
                value[static_cast<int64_t>(token) * layout.token_stride[VALUE] + head * layout.head_stride[VALUE] + dimension])));
        }
    }
...
__global__ void quantize_value(const __nv_bfloat16* value, uint8_t* output, const float* scales,
    ...
    for (int part = 0; part < 32; part += 8) {
        ...
        const float v = token < tokens ? __bfloat162float(
            value[static_cast<int64_t>(token) * layout.token_stride[VALUE] + head * layout.head_stride[VALUE] + dimension]) / scale : 0.0f;
    ...
    for (int part = 0; part < 32; part += 8) {
        ...
        output[(static_cast<int64_t>(head) * HEAD_DIM + dimension) * padded + token] = tile[x][y + part];
    }
}

__global__ void quantize_qk(const __nv_bfloat16* query, const __nv_bfloat16* key, uint8_t* query8, uint8_t* key8,
                             float* query_scales, float* key_scales, int tokens, int heads, Mmh3AttentionLayout layout) {
    __shared__ float scratch[8];
    const int block = blockIdx.x;
    const int head = blockIdx.y;
    ...
    for (int index = threadIdx.x; index < 64 * HEAD_DIM; index += 256) {
        ...
        if (token < tokens) {
            maximum = fmaxf(maximum, fabsf(__bfloat162float(
                source[static_cast<int64_t>(token) * layout.token_stride[part] + head * layout.head_stride[part] + dimension])));
        }
    }
    for (int offset = 16; offset > 0; offset /= 2) {
        maximum = fmaxf(maximum, __shfl_xor_sync(0xffffffff, maximum, offset));
    ...
    for (int index = threadIdx.x; index < 64 * HEAD_DIM; index += 256) {
        ...
        if (token < tokens) {
            const float value = __bfloat162float(
                source[static_cast<int64_t>(token) * layout.token_stride[part] + head * layout.head_stride[part] + dimension]) / scale;
...
__global__ void __launch_bounds__(THREADS, 2)
    ...
    load_qk8(stage_address(1), query_head, layout.token_stride[QUERY], first_query, BLOCK_M, tokens);
    copy_async_commit();
...
#pragma unroll
    for (int half = 0; half < 2; half++) {
        ...
        offsets[half] = SPARSE && token < tokens ? workspace.row_offsets[static_cast<size_t>(head) * tokens + token] : 0.0f;
    }
...
#pragma unroll
            for (int key_pair = 0; key_pair < BLOCK_N / 16; key_pair++) {
                ...
                mma_qk(raw_scores[key_pair * 2], query_fragments[k_step], registers[0], registers[1]);
                mma_qk(raw_scores[key_pair * 2 + 1], query_fragments[k_step], registers[2], registers[3]);
            }
        }
...
#pragma unroll
            for (int element = 0; element < 4; element++) {
                float score = static_cast<float>(raw_scores[key_tile_index][element]) *
                    (scale_log2 * q_scales[head * route_blocks + (first_query + warp * 16) / 64] *
                     k_scales[head * route_blocks + key_block]) - offsets[element / 2];
                if (masked_block) {
                    const int key_index = key_block * BLOCK_N + key_tile_index * 8 + (lane % 4) * 2 + (element % 2);
                    if (key_index >= tokens) {
                        score = -FLT_MAX;
                    }
                }
                ...
                for (int half = 0; half < 2; half++) {
                    uint32_t pairs = static_cast<uint32_t>(__nv_cvt_float2_to_fp8x2(
                        make_float2(scores[tile_base][half * 2] * 448.0f,
                                    scores[tile_base][half * 2 + 1] * 448.0f), __NV_SATFINITE, __NV_E4M3));
                    pairs |= static_cast<uint32_t>(__nv_cvt_float2_to_fp8x2(
                        make_float2(scores[tile_base + 1][half * 2] * 448.0f,
                                    scores[tile_base + 1][half * 2 + 1] * 448.0f), __NV_SATFINITE, __NV_E4M3)) << 16;
                    const uint32_t first = __shfl_sync(0xffffffff, pairs, source_lane) >> ((lane & 2) * 8);
                    const uint32_t second = __shfl_sync(0xffffffff, pairs, source_lane + 1) >> ((lane & 2) * 8);
...
#pragma unroll
            for (int dimension_tile = 0; dimension_tile < HEAD_DIM / 8; dimension_tile++) {
                ...
                const float first = (output_accumulators[dimension_tile][half * 2] * value_factor * routed_weight +
                                     tail_values[dimension] * tail_weight) * inverse_sum;
                const float second = (output_accumulators[dimension_tile][half * 2 + 1] * value_factor * routed_weight +
...
int launch_attention(const Mmh3QuantizedWorkspace& workspace, __nv_bfloat16* output, int tokens, int heads,
    ...
    attention_kernel<SPARSE><<<dim3((tokens + 63) / 64, heads), THREADS, SHARED_BYTES, stream>>>(
        workspace.query, workspace.key, workspace.value, output, tokens, layout, scale * 1.4426950408889634f,
        workspace.query_scales, workspace.key_scales, workspace.value_scales, sparse);
