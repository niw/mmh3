//! Shapes of one generation request: the frame grid the video VAE decodes and the latent sizes that match it.
...
/// Frame counts are `17n + 5`: the VAE encodes clips of 17 frames and keeps 5 latent frames of each.
...
impl GenerationShape {
    /// The shape for a canvas of `width × height` and at least `frames` frames, snapped up to the `17n + 5` grid.
    pub fn new(width: usize, height: usize, frames: usize) -> Result<Self, String> {
        if width == 0 || height == 0 || width % CANVAS_MULTIPLE != 0 || height % CANVAS_MULTIPLE != 0 {
            return Err(format!("width and height must be positive multiples of {CANVAS_MULTIPLE}, got {width}×{height}"));
        }
        ...
        let audio_frames = (frames as f64 / FPS as f64 * AUDIO_LATENTS_PER_SECOND as f64).round_ties_even() as usize;
        Ok(GenerationShape { width, height, frames, latent_frames, audio_frames })
    }

    /// `[channels, frames, height, width]` of the video latent.
    pub fn video_latent_shape(&self) -> Vec<usize> {
        vec![VIDEO_LATENT_CHANNELS, self.latent_frames, self.height / SPATIAL_RATIO, self.width / SPATIAL_RATIO]
    }
...
mod tests {
    ...
    fn snaps_to_the_reference_grid() {
        ...
        assert_eq!((shape.frames, shape.latent_frames, shape.audio_frames), (124, 37, 207));
        ...
        assert_eq!((small.frames, small.latent_frames, small.audio_frames), (22, 7, 37));
