...
fn linear(input: &[f32], weight: &Tensor, bias: Option<&Tensor>) -> Vec<f32> {
    ...
    assert_eq!(input.len() % width, 0, "input width does not match the weight");
    ...
    for row in input.chunks_exact(width) {
        for (output, weight_row) in weight.data.chunks_exact(width).enumerate() {
            let dot: f64 = row.iter().zip(weight_row).map(|(&left, &right)| left as f64 * right as f64).sum();
...
fn rms_norm(data: &mut [f32], width: usize, weight: &[f32], epsilon: f32) {
    for row in data.chunks_exact_mut(width) {
        let mean_square = row.iter().map(|&value| value as f64 * value as f64).sum::<f64>() / width as f64;
...
fn attention(query: &[f32], key: &[f32], value: &[f32], heads: usize, head_dim: usize) -> Vec<f32> {
    ...
    for head in 0..heads {
        let range = |token: usize| token * inner + head * head_dim..token * inner + (head + 1) * head_dim;
        for query_token in 0..tokens {
            ...
            for (key_token, score) in scores.iter_mut().enumerate() {
                ...
                *score = query_row.iter().zip(key_row).map(|(&left, &right)| left as f64 * right as f64).sum::<f64>() * scale;
            }
            ...
            for dimension in 0..head_dim {
                let sum: f64 = (0..tokens).map(|key_token| scores[key_token] * value[key_token * inner + head * head_dim + dimension] as f64).sum();
...
/// Rotates dimension pairs (i, i + half) for i < half within the first 2 × half dimensions of every head.
/// `angles` is `[tokens, half]`.
fn apply_rope(data: &mut [f32], heads: usize, head_dim: usize, angles: &[f32]) {
    ...
    for (token_index, token) in data.chunks_exact_mut(heads * head_dim).enumerate() {
        for head in token.chunks_exact_mut(head_dim) {
            for (pair, &angle) in angles[token_index * half..(token_index + 1) * half].iter().enumerate() {
...
fn self_attention(hidden: &[f32], weights: &AttentionWeights, config: &DitConfig, angles: Option<&[f32]>) -> Vec<f32> {
    ...
    rms_norm(&mut query, config.head_dim, &weights.query_norm.data, config.norm_eps);
    rms_norm(&mut key, config.head_dim, &weights.key_norm.data, config.norm_eps);
    ...
    linear(&attention(&query, &key, &value, config.heads, config.head_dim), weights.output, None)
}
...
fn feed_forward(hidden: &[f32], first: &Tensor, second: &Tensor, ffn: usize) -> Vec<f32> {
    ...
    let activated: Vec<f32> = projected
        .chunks_exact(2 * ffn)
        .flat_map(|row| row[..ffn].iter().zip(&row[ffn..]).map(|(&gate, &up)| silu(gate) * up).collect::<Vec<_>>())
        .collect();
...
fn refine_text(weights: &DitWeights, config: &DitConfig, context: &Tensor) -> Vec<f32> {
    ...
    let mut hidden = linear(&context.data, weights.get("condition_proj.weight"), Some(weights.get("condition_proj.bias")));
    for layer in 0..config.refiner_layers {
        let prefix = format!("token_refiner.blocks.{layer}");
        let mut normalized = hidden.clone();
        rms_norm(&mut normalized, width, &weights.get(&format!("{prefix}.norm1.weight")).data, config.norm_eps);
        let attended = self_attention(&normalized, &attention_weights(weights, &prefix), config, None);
        hidden.iter_mut().zip(&attended).for_each(|(value, &delta)| *value += delta);
        let mut normalized = hidden.clone();
        rms_norm(&mut normalized, width, &weights.get(&format!("{prefix}.norm2.weight")).data, config.norm_eps);
        let mixed = feed_forward(&normalized, weights.get(&format!("{prefix}.mlp.fc1.weight")), weights.get(&format!("{prefix}.mlp.fc2.weight")), config.ffn);
        hidden.iter_mut().zip(&mixed).for_each(|(value, &delta)| *value += delta);
    }
    rms_norm(&mut hidden, width, &weights.get("token_refiner.final_norm.weight").data, config.norm_eps);
    hidden
}

/// Scale-and-shift modulation vectors of one AdaLN projection: `[timesteps × modalities, chunks, hidden]`.
struct Modulation {
    values: Vec<f32>,
    chunks: usize,
    hidden: usize,
...
impl Modulation {
    fn new(time_embedding: &[f32], weight: &Tensor, bias: &Tensor, chunks: usize, hidden: usize) -> Self {
        Modulation { values: linear(time_embedding, weight, Some(bias)), chunks, hidden }
    }
...
pub fn forward(weights: &DitWeights, config: &DitConfig, inputs: &DitInputs) -> DitTrace {
    ...
    let layout = PackedLayout::text_to_video(inputs.context.shape[0], video_shape[1], video_shape[2], video_shape[3], inputs.audio.shape[2]);
    ...
    let audio_embed = linear(&pack_audio(&inputs.audio), weights.get("audio_patch_proj.weight"), Some(weights.get("audio_patch_proj.bias")));
    let video_embed = linear(&patchify_video(&inputs.video), weights.get("video_patch_proj.weight"), Some(weights.get("video_patch_proj.bias")));
    ...
    let modulate = |normalized: &mut [f32], modulation: &Modulation, shift_chunk: usize, scale_chunk: usize| {
        for (token, row) in normalized.chunks_exact_mut(width).enumerate() {
            let (shift, scale) = (modulation.vector(rows[token], shift_chunk), modulation.vector(rows[token], scale_chunk));
            for index in 0..width {
                row[index] = row[index] * (1.0 + scale[index]) + shift[index];
            }
        }
    };
    let add_gated = |hidden: &mut [f32], delta: &[f32], modulation: &Modulation, gate_chunk: usize| {
        for (token, (row, delta_row)) in hidden.chunks_exact_mut(width).zip(delta.chunks_exact(width)).enumerate() {
    ...
    for layer in 0..config.layers {
        ...
        let attended = self_attention(&normalized, &attention_weights(weights, &prefix), config, Some(&angles));
    ...
    let project = |kind: SegmentKind, head: &str| {
        ...
        rms_norm(&mut rows, width, &weights.get("final_layer.norm.weight").data, config.norm_eps);
        let timestep = timesteps.index_of(kind);
        let (shift, scale) = (final_modulation.vector(timestep, 0), final_modulation.vector(timestep, 1));
        for row in rows.chunks_exact_mut(width) {
            for index in 0..width {
                row[index] = row[index] * (1.0 + scale[index]) + shift[index];
            }
        }
        linear(&rows, weights.get(&format!("final_layer.{head}.weight")), Some(weights.get(&format!("final_layer.{head}.bias"))))
    };
    let video_rows = project(SegmentKind::Video, "video_out");
    let audio_rows = project(SegmentKind::Audio, "audio_out");
    let video = unpatchify_video(&video_rows, video_shape).into_iter().map(|value| -value).collect();
    let audio = unpack_audio(&audio_rows, &inputs.audio.shape).into_iter().map(|value| -value).collect();
    DitTrace { text_states, block_outputs, video, audio }
}
