//! Uncompressed output files: YUV4MPEG2 video and WAV audio, which common tools such as ffmpeg read
//! directly.
...
impl Yuv420 {
    ...
    /// Converts pixels `[3, frames, height, width]` in [0, 1]. Chroma is the average of each 2 × 2
    /// block. Height and width must be even.
    pub fn from_pixels(pixels: &Tensor) -> io::Result<Self> {
        let &[channels, frames, height, width] = pixels.shape.as_slice() else {
            return Err(io::Error::new(
                io::ErrorKind::InvalidInput,
                "pixels must be [3, frames, height, width]",
            ));
        };
        if channels != 3 || height % 2 != 0 || width % 2 != 0 {
            return Err(io::Error::new(
                io::ErrorKind::InvalidInput,
                "pixels must be RGB with an even height and width",
            ));
        }
        ...
        for frame in 0..frames {
            ...
            data.extend((0..plane).map(|index| {
                limited_range(
                    LUMA_RED * r[index] + luma_green * g[index] + LUMA_BLUE * b[index],
                    16.0,
                    219.0,
                )
            }));
            ...
            for y in 0..height / 2 {
                for x in 0..width / 2 {
                    ...
                    let average = |values: &[f32]| {
                        corners.iter().map(|&index| values[index]).sum::<f32>() / 4.0
                    };
                    let (red_value, green_value, blue_value) = (average(r), average(g), average(b));
                    let luma_value =
                        LUMA_RED * red_value + luma_green * green_value + LUMA_BLUE * blue_value;
                    blue[y * width / 2 + x] = limited_range(
                        (blue_value - luma_value) / (2.0 * (1.0 - LUMA_BLUE)),
                        128.0,
                        224.0,
                    );
                    red[y * width / 2 + x] = limited_range(
                        (red_value - luma_value) / (2.0 * (1.0 - LUMA_RED)),
                        128.0,
                        224.0,
                    );
                }
...
pub fn write_wav(writer: &mut impl Write, waveform: &Tensor, sample_rate: usize) -> io::Result<()> {
    let &[channels, samples] = waveform.shape.as_slice() else {
        return Err(io::Error::new(
            io::ErrorKind::InvalidInput,
            "the waveform must be [channels, samples]",
        ));
    };
    ...
    for sample in 0..samples {
        for channel in 0..channels {
            let value = (waveform.data[channel * samples + sample].clamp(-1.0, 1.0) * 32767.0)
                .round() as i16;
...
mod tests {
    ...
    fn writes_y4m_frames() {
        ...
        write_y4m(
            &mut bytes,
            &Yuv420::from_pixels(&Tensor::new(vec![3, 2, 2, 2], data)).unwrap(),
            24,
        )
        .unwrap();
    ...
    fn writes_pcm_wav() {
        ...
        write_wav(
            &mut bytes,
            &Tensor::new(vec![2, 2], vec![0.5, -1.0, 1.0, 2.0]),
            32_000,
        )
        .unwrap();
