//! The MiniMax H3 text encoder on the GPU: the first 50 layers of the Qwen3-VL-32B language model.
//! The hidden state after the last layer, without the final norm, conditions the DiT.
//!
...
impl CudaTextEncoder {
    ...
    pub fn load(file: &SafeTensors) -> Result<Self, Error> {
        ...
        let config = TextEncoderConfig {
            ...
            layers: (0..)
                .take_while(|layer| {
                    file.get(&format!("{PREFIX}layers.{layer}.input_layernorm.weight"))
                        .is_some()
                })
                .count(),
    ...
    fn insert_concatenated(
        ...
        for part in parts {
            ...
            if weight.dtype != DType::I8
                ...
                || scale.element_count() != weight.shape[0]
            {
                return Err(Error::Model(format!(
                    "{part} is not a per-channel INT8 layer"
                )));
            }
            if weights
                .first()
                .is_some_and(|first: &&TensorInfo| first.shape[1] != weight.shape[1])
            {
                return Err(Error::Model(format!(
                    "{part} does not share its input width with {name}"
                )));
            }
            weights.push(weight);
            scales.push(scale);
    ...
    fn encode_positions(
        ...
        if tokens == 0 || ids.iter().any(|&id| id as usize >= config.vocabulary) {
            return Err(Error::Model(format!(
                "token ids must be non-empty and below {}",
                config.vocabulary
            )));
        }
        ...
        let layout = AttentionLayout {
            token_stride: [
                ...
                qkv_width as i64,
                qkv_width as i64,
                (config.query_heads * HEAD_DIM) as i64,
            ],
        ...
        for layer in 0..config.layers {
            ...
            // SAFETY: qkv holds `tokens` rows of [q | k | v], the angles cover every token and the
            // attention output holds `tokens × query heads × 128` values.
            unsafe {
        ...
        Ok(TextEncoding {
            context: Tensor::new(vec![tokens, config.hidden], workspace.residual.to_f32()?),
            layers,
        })
    }
}
