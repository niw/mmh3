...
fn check_forward(precision: AttentionPrecision) {
    ...
    assert_close("text states", &outputs.text_states, &tensor(&file, "intermediate.text_states").data);
    for (index, block) in &outputs.blocks {
        assert_close(&format!("block {index}"), block, &tensor(&file, &format!("intermediate.block.{index}")).data);
    }
