...
extern "C" int mmh3_bench_cublaslt_gemm(int kind, int64_t m, int64_t n, int64_t k, int iterations,
                                        float *best_milliseconds, int *algorithm_count,
    ...
    *algorithm_count = 0;
    ...
    if ((status = cublasLtMatmulDescCreate(&lt.operation, compute_type, scale_type)) !=
    ...
    cublasLtMatmulDescSetAttribute(lt.operation, CUBLASLT_MATMUL_DESC_TRANSA, &transpose,
                                   sizeof(transpose));
    cublasLtMatmulDescSetAttribute(lt.operation, CUBLASLT_MATMUL_DESC_TRANSB, &no_transpose,
                                   sizeof(no_transpose));

    if (kind == GEMM_KIND_NVFP4) {
        // NOTE: block scales use cuBLASLt's swizzled layout, which needs 128-row by 4-column
        // padding. Their values do not affect timing, so the buffers are only sized and filled.
        ...
        if ((cuda_status = cudaMalloc(&weight_scales.pointer, weight_scale_bytes)) != cudaSuccess ||
            (cuda_status = cudaMalloc(&activation_scales.pointer, activation_scale_bytes)) !=
                cudaSuccess) {
            return cuda_failure(cuda_status, "cudaMalloc scales", message, message_size);
        }
        if ((cuda_status = cudaMemset(weight_scales.pointer, 0x38, weight_scale_bytes)) !=
                cudaSuccess ||
            (cuda_status = cudaMemset(activation_scales.pointer, 0x38, activation_scale_bytes)) !=
                cudaSuccess) {
        ...
        cublasLtMatmulDescSetAttribute(lt.operation, CUBLASLT_MATMUL_DESC_A_SCALE_MODE, &scale_mode,
                                       sizeof(scale_mode));
        cublasLtMatmulDescSetAttribute(lt.operation, CUBLASLT_MATMUL_DESC_B_SCALE_MODE, &scale_mode,
                                       sizeof(scale_mode));
        cublasLtMatmulDescSetAttribute(lt.operation, CUBLASLT_MATMUL_DESC_A_SCALE_POINTER,
                                       &weight_scales.pointer, sizeof(weight_scales.pointer));
        cublasLtMatmulDescSetAttribute(lt.operation, CUBLASLT_MATMUL_DESC_B_SCALE_POINTER,
    ...
    if ((status = cublasLtMatrixLayoutCreate(&lt.layout_a, input_type, k, n, k)) !=
            CUBLAS_STATUS_SUCCESS ||
        (status = cublasLtMatrixLayoutCreate(&lt.layout_b, input_type, k, m, k)) !=
            CUBLAS_STATUS_SUCCESS ||
        (status = cublasLtMatrixLayoutCreate(&lt.layout_d, output_type, n, m, n)) !=
            CUBLAS_STATUS_SUCCESS) {
    ...
    cublasLtMatmulPreferenceSetAttribute(lt.preference, CUBLASLT_MATMUL_PREF_MAX_WORKSPACE_BYTES,
                                         &workspace_bytes, sizeof(workspace_bytes));

    cublasLtMatmulHeuristicResult_t results[REQUESTED_ALGORITHMS];
    int returned = 0;
    status = cublasLtMatmulAlgoGetHeuristic(lt.handle, lt.operation, lt.layout_a, lt.layout_b,
                                            lt.layout_d, lt.layout_d, lt.preference,
                                            REQUESTED_ALGORITHMS, results, &returned);
    ...
    const void *alpha =
        kind == GEMM_KIND_INT8 ? static_cast<const void *>(&alpha_int) : &alpha_float;
    ...
    for (int index = 0; index < returned; index++) {
        ...
        for (int run = 0; run < WARMUP_RUNS && usable; run++) {
            usable =
                cublasLtMatmul(lt.handle, lt.operation, alpha, weight.pointer, lt.layout_a,
                               activation.pointer, lt.layout_b, beta, output.pointer, lt.layout_d,
                               output.pointer, lt.layout_d, algorithm, workspace.pointer,
                               WORKSPACE_BYTES, nullptr) == CUBLAS_STATUS_SUCCESS;
        }
...
extern "C" int mmh3_bench_memory_copy(size_t bytes, int iterations,
                                      float *kernel_gigabytes_per_second,
                                      float *memcpy_gigabytes_per_second, char *message,
                                      size_t message_size) {
    ...
    copy_kernel<<<blocks, 256>>>(static_cast<const uint4 *>(source.pointer),
                                 static_cast<uint4 *>(destination.pointer), count);
    cudaEventRecord(events.start);
    for (int run = 0; run < iterations; run++) {
        copy_kernel<<<blocks, 256>>>(static_cast<const uint4 *>(source.pointer),
                                     static_cast<uint4 *>(destination.pointer), count);
    ...
    *kernel_gigabytes_per_second =
        static_cast<float>(2.0 * bytes * iterations / (elapsed * 1e-3) / 1e9);
...
extern "C" int mmh3_bench_attention(int tokens, int heads, int iterations, float *milliseconds,
                                    char *message, size_t message_size) {
    const int64_t inner = static_cast<int64_t>(heads) * 128;
    DeviceMemory qkv, output;
    cudaError_t status;
    if ((status = cudaMalloc(&qkv.pointer, tokens * 3 * inner * sizeof(__nv_bfloat16))) !=
            cudaSuccess ||
        (status = cudaMalloc(&output.pointer, tokens * inner * sizeof(__nv_bfloat16))) !=
            cudaSuccess) {
        return cuda_failure(status, "cudaMalloc", message, message_size);
    }
    if ((status = fill_pattern(qkv.pointer, tokens * 3 * inner * sizeof(__nv_bfloat16), 8)) !=
        cudaSuccess) {
        return cuda_failure(status, "fill", message, message_size);
    }
    const __nv_bfloat16 *base = static_cast<const __nv_bfloat16 *>(qkv.pointer);
    auto run = [&]() {
        return mmh3_attention_bf16(
            base, base + inner, base + 2 * inner, static_cast<__nv_bfloat16 *>(output.pointer),
            tokens, heads, 3 * inner, 3 * inner, 3 * inner, inner, 0.08838834764831845f, nullptr);
    };
    int code = run();
    if (code != 0 || (status = cudaDeviceSynchronize()) != cudaSuccess) {
        return cuda_failure(code != 0 ? static_cast<cudaError_t>(code) : status, "attention",
