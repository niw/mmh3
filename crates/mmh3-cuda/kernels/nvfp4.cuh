...
// Quantizes the eight values this lane holds of 256-column group `group` of row `row`, the block
// shared with the neighbouring lane, and stores them with their block scale.
__device__ __forceinline__ void nvfp4_store_group(const float (&values)[8], int lane, int row,
    ...
    const __nv_fp8_storage_t scale = __nv_cvt_float_to_fp8(
        maximum / NVFP4_VALUE_MAX * inverse_tensor_scale, __NV_SATFINITE, __NV_E4M3);
