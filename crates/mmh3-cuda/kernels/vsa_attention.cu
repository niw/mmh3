...
//
// 1. pool: per head and tile, the mean query, key and value.
...
struct Mmh3VsaWorkspace {
    ...
    float *pooled_value;         // [heads, tiles, 128]
...
__global__ void __launch_bounds__(THREADS)
    ...
    workspace.pooled_value[index] = value_sum / rows;
}
...
__global__ void __launch_bounds__(THREADS)
    ...
    __shared__ int histogram[RADIX_BINS];
    __shared__ uint32_t threshold_bits;
    __shared__ int threshold_ties;
    ...
    for (int tile = warp; tile < tiles; tile += THREADS / 32) {
        float loaded[HEAD / 32];
        #pragma unroll
        for (int part = 0; part < HEAD / 32; part++) {
            loaded[part] = keys[static_cast<size_t>(tile) * HEAD + lane + part * 32];
        }
        #pragma unroll
        for (int query = 0; query < QUERIES; query++) {
            float partial = 0.0f;
            #pragma unroll
            for (int part = 0; part < HEAD / 32; part++) {
                partial = fmaf(queries_pooled[query][lane + part * 32], loaded[part], partial);
            }
            partial = warp_sum(partial);
            if (lane == 0) {
                scores[query * tiles + tile] = partial * scale;
            }
        }
    }
    __syncthreads();
    ...
    for (int query = 0; query < queries; query++) {
        ...
        if (!dense) {
            // Radix select of the kept-th largest video score, eight bits at a time.
            uint32_t prefix = 0, mask = 0;
            int remaining = kept;
            for (int shift = 24; shift >= 0; shift -= 8) {
                for (int bin = threadIdx.x; bin < RADIX_BINS; bin += THREADS) {
                    histogram[bin] = 0;
                }
                __syncthreads();
                for (int tile = prefix_tiles + threadIdx.x; tile < tiles; tile += THREADS) {
                    const uint32_t bits = ordered_bits(query_scores[tile]);
                    if ((bits & mask) == prefix) {
                        atomicAdd(&histogram[(bits >> shift) & 0xff], 1);
                    }
                }
                __syncthreads();
                if (threadIdx.x == 0) {
                    int bin = RADIX_BINS - 1;
                    while (histogram[bin] < remaining) {
                        remaining -= histogram[bin];
                        bin--;
                    }
                    threshold_bits = prefix | (static_cast<uint32_t>(bin) << shift);
                    threshold_ties = remaining;
                }
                __syncthreads();
                prefix = threshold_bits;
                remaining = threshold_ties;
                mask |= 0xffu << shift;
                __syncthreads();
            }
        }
        if (warp == 0) {
            ...
            for (int start = 0; start < tiles; start += 32) {
                ...
                const bool tie = candidate && bits == threshold_bits;
                const unsigned tie_mask = __ballot_sync(0xffffffff, tie);
                const int tie_rank = ties + __popc(tie_mask & ((1u << lane) - 1));
                ties += __popc(tie_mask);
                const bool flag = candidate
                                      ? bits > threshold_bits || (tie && tie_rank < threshold_ties)
                                      : tile < tiles;
                const unsigned selected = __ballot_sync(0xffffffff, flag);
                if (flag) {
                    route[count + __popc(selected & ((1u << lane) - 1))] =
    ...
    for (int tile = 0; tile < tiles; tile++) {
        const float loaded = values[static_cast<size_t>(tile) * HEAD];
        #pragma unroll
        for (int query = 0; query < QUERIES; query++) {
            outputs[query] = fmaf(scores[query * tiles + tile], loaded, outputs[query]);
        }
    }
    #pragma unroll
    for (int query = 0; query < QUERIES; query++) {
...
extern "C" int mmh3_vsa_attention(const void *query, const void *key, const void *value,
                                  const void *gate, int64_t gate_stride, void *output, int heads,
                                  const Mmh3AttentionLayout *layout, float scale, int tiles,
                                  int prefix_tiles, int kept, const Mmh3VsaWorkspace *workspace,
                                  cudaStream_t stream) {
    if (heads <= 0 || tiles <= 0 || tiles > 65536 || prefix_tiles < 0 || prefix_tiles > tiles ||
        kept < 1) {
    ...
    pool_kernel<<<dim3(tiles, heads), THREADS, 0, stream>>>(q, k, v, *layout, tiles, *workspace);
