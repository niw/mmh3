...
fn check_against_reference(tokens: usize, heads: usize) {
    ...
    let qkv: Vec<u16> = (0..tokens * 3 * inner)
        .map(|_| to_bf16(random.uniform(-2.0, 2.0)))
        .collect();
        ...
        for query in 0..tokens {
            let scores: Vec<f64> = (0..tokens)
                .map(|key| {
                    (0..HEAD_DIM)
                        .map(|dimension| {
                            element(query, 0, head, dimension) * element(key, 1, head, dimension)
                        })
                        .sum::<f64>()
                        * scale as f64
                })
            ...
            for dimension in 0..HEAD_DIM {
                let expected: f64 = (0..tokens)
                    .map(|key| weights[key] * element(key, 2, head, dimension))
                    .sum::<f64>()
                    / total;
...
fn to_f16(value: f32) -> u16 {
    ...
    let (exponent, rounded) = if rounded == 1024 {
        (exponent + 1, 0)
    } else {
        (exponent, rounded)
    };
...
/// The VAE decoder layout: FP16, heads of 64 with [q, k, v] interleaved per head, several
/// independent tiles.
#[test]
fn matches_cpu_reference_for_interleaved_half_precision_heads() {
    ...
    let values: Vec<u16> = (0..batch * tokens * per_token)
        .map(|_| to_f16(random.uniform(-2.0, 2.0)))
        .collect();
    let input = {
    ...
    let layout = AttentionLayout {
        ...
        batch_stride: [
            (tokens * per_token) as i64,
            (tokens * per_token) as i64,
            (tokens * per_token) as i64,
            (tokens * output_width) as i64,
        ],
            ...
            for query in 0..tokens {
                let scores: Vec<f64> = (0..tokens)
                    .map(|key| {
                        (0..head_dim)
                            .map(|d| {
                                element(tile, query, head, 0, d) * element(tile, key, head, 1, d)
                            })
                            .sum::<f64>()
                            * scale as f64
                    })
                ...
                for dimension in 0..head_dim {
                    let expected: f64 = (0..tokens)
                        .map(|key| weights[key] * element(tile, key, head, 2, dimension))
                        .sum::<f64>()
                        / total;
                    let offset =
                        ((tile * tokens + query) * output_width + head * head_dim + dimension) * 2;
                    let actual =
                        f16_to_f32(u16::from_le_bytes([bytes[offset], bytes[offset + 1]])) as f64;
                    assert!(
                        (actual - expected).abs() <= 0.01 + 0.005 * expected.abs(),
                        "tile {tile}, head {head}, query {query}, dimension {dimension}: expected {expected}, got {actual}"
                    );
                }
            }
        }
...
/// The text encoder layout: BF16 [q | k | v] rows with eight query heads per key and value head,
/// causal.
#[test]
fn matches_cpu_reference_for_causal_grouped_heads() {
    ...
    let values: Vec<u16> = (0..tokens * row)
        .map(|_| to_bf16(random.uniform(-2.0, 2.0)))
        .collect();
    let input = {
        let mut buffer = DeviceBuffer::new(values.len() * 2).unwrap();
    ...
    let offsets = AttentionOffsets {
        query: 0,
        key: query_heads * HEAD_DIM,
        value: (query_heads + key_heads) * HEAD_DIM,
        output: 0,
    };
    let scale = 1.0 / (HEAD_DIM as f32).sqrt();
    attention::attention(
    ...
    for head in 0..query_heads {
        let (key_column, value_column) = (
            (query_heads + head / group) * HEAD_DIM,
            (query_heads + key_heads + head / group) * HEAD_DIM,
        );
        for query in 0..tokens {
            let scores: Vec<f64> = (0..=query)
                .map(|key| {
                    (0..HEAD_DIM)
                        .map(|d| element(query, head * HEAD_DIM + d) * element(key, key_column + d))
                        .sum::<f64>()
                        * scale as f64
                })
                .collect();
            ...
            let total: f64 = weights.iter().sum();
            for dimension in 0..HEAD_DIM {
                let expected: f64 = (0..=query)
                    .map(|key| weights[key] * element(key, value_column + dimension))
                    .sum::<f64>()
                    / total;
                let offset = (query * output_width + head * HEAD_DIM + dimension) * 2;
                let actual =
                    from_bf16(u16::from_le_bytes([bytes[offset], bytes[offset + 1]])) as f64;
                assert!(
                    (actual - expected).abs() <= 0.02 + 0.01 * expected.abs(),
                    "head {head}, query {query}, dimension {dimension}: expected {expected}, got {actual}"
                );
            }
        }
    }
