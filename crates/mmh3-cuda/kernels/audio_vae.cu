...
// Kernels of the audio VAE's BigVGAN decoder. Signals are FP32 and time-major, [sequences, length,
// channels], so convolutions become GEMMs over im2col rows.
...
// Transposed convolution from its per-tap products: products[(s, i), j, c] = Σ input[s, i, :] ·
// weight[:, c, j]. output[s, t, c] = bias[c] + Σ products[s, i, j, c] over the taps j with i ·
// stride = t + padding − j.
__global__ void conv_transpose_gather_kernel(const float *__restrict__ products,
                                             const float *__restrict__ bias,
                                             float *__restrict__ output, int sequences,
                                             int input_length, int output_length, int channels,
                                             int kernel, int stride_size, int padding) {
    ...
    for (size_t index = static_cast<size_t>(blockIdx.x) * blockDim.x + threadIdx.x; index < count;
        ...
        for (int tap = (time + padding) % stride_size; tap < kernel; tap += stride_size) {
            ...
            if (source < input_length) {
                sum += products[((sequence * input_length + source) * kernel + tap) * channels +
                                channel];
            }
        }
        output[index] = sum;
...
// Upsampled signal at position `position` of 2 × length: replicate-padded transposed convolution
// with the upsample filter, scaled by 2 and cropped.
__device__ __forceinline__ float upsampled(const float *signal, int length, int channels,
                                           int channel, int position, const float *filter) {
...
// SnakeBeta between a 2× upsample and a 2× downsample: x + sin²(αx) / β on the upsampled signal,
// then the replicate-padded low-pass filter with stride 2. `parameters` holds α and 1 / (β + 1e-9)
// per channel.
__global__ void snake_beta_kernel(const float *__restrict__ input, float *__restrict__ output,
                                  ...
                                  const float *__restrict__ down_filter, int sequences, int length,
                                  int channels) {
    __shared__ float filters[2 * RESAMPLE_TAPS];
    if (threadIdx.x < 2 * RESAMPLE_TAPS) {
        filters[threadIdx.x] = threadIdx.x < RESAMPLE_TAPS
                                   ? up_filter[threadIdx.x]
                                   : down_filter[threadIdx.x - RESAMPLE_TAPS];
    }
    __syncthreads();
...
__global__ void add_kernel(float *output, const float *first, const float *second, size_t count,
                           float divisor) {
    const size_t stride = static_cast<size_t>(gridDim.x) * blockDim.x;
    for (size_t index = static_cast<size_t>(blockIdx.x) * blockDim.x + threadIdx.x; index < count;
         index += stride) {
...
extern "C" int mmh3_audio_conv_transpose_gather(const float *products, const float *bias,
                                                float *output, int sequences, int input_length,
                                                int output_length, int channels, int kernel,
                                                int stride, int padding, cudaStream_t stream) {
    const size_t count = static_cast<size_t>(sequences) * output_length * channels;
    conv_transpose_gather_kernel<<<grid_for(count), THREADS, 0, stream>>>(
        products, bias, output, sequences, input_length, output_length, channels, kernel, stride,
        padding);
    return static_cast<int>(cudaGetLastError());
}

extern "C" int mmh3_audio_snake_beta(const float *input, float *output, const float *parameters,
                                     const float *up_filter, const float *down_filter,
                                     int sequences, int length, int channels, cudaStream_t stream) {
    const size_t count = static_cast<size_t>(sequences) * length * channels;
    snake_beta_kernel<<<grid_for(count), THREADS, 0, stream>>>(
        input, output, parameters, up_filter, down_filter, sequences, length, channels);
    return static_cast<int>(cudaGetLastError());
}

extern "C" int mmh3_audio_add(float *output, const float *first, const float *second, size_t count,
                              float divisor, cudaStream_t stream) {
