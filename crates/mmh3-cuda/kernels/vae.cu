...
__global__ void __launch_bounds__(ROW_THREADS)
    norm_kernel(const float* __restrict__ input, const __half* __restrict__ weight, const __half* __restrict__ bias,
                __half* __restrict__ output, int width, float epsilon) {
...
// Per-head RMSNorm without weight of q and k in the [tokens, heads, 3, 64] qkv buffer, then the split-half rotary
// embedding on the first 2 × pairs dimensions. Angles are per token within a tile.
__global__ void qk_norm_rope_kernel(__half* qkv, const float* __restrict__ angles, int pairs, int tile_tokens,
                                    int tokens, int heads, float epsilon) {
...
__global__ void residual_add_scaled_kernel(float* __restrict__ residual, const __half* __restrict__ delta,
                                           const __half* __restrict__ scale, size_t count, int width) {
...
__global__ void swiglu_kernel(const __half* __restrict__ input, __half* __restrict__ output, size_t count, int width) {
    const size_t stride = static_cast<size_t>(gridDim.x) * blockDim.x;
    for (size_t index = static_cast<size_t>(blockIdx.x) * blockDim.x + threadIdx.x; index < count; index += stride) {
...
// Widens the embedded tokens to FP32 and writes the register tokens and the zero token after each tile's patches.
__global__ void embed_suffix_kernel(const __half* __restrict__ embedded, const __half* __restrict__ registers,
                                    float* __restrict__ residual, int tile_tokens, int patches, int registers_count,
...
__device__ __forceinline__ float tile_pixel(const __half* projected, const TileGrid& grid, int tile, int channel,
                                            int frame, int row, int column) {
    const int latent_height = grid.tile_height / PATCH;
    const int latent_width = grid.tile_width / PATCH;
    const int token = ((frame / PATCH_FRAMES) * latent_height + row / PATCH) * latent_width + column / PATCH;
    const int feature = ((channel * PATCH_FRAMES + frame % PATCH_FRAMES) * PATCH + row % PATCH) * PATCH + column % PATCH;
    const int features = 3 * PATCH_FRAMES * PATCH * PATCH;
    return __half2float(projected[(static_cast<size_t>(tile) * grid.tile_tokens + token) * features + feature]);
}

// Assembles one chunk's canvas [3, frames, height, width] from the decoded tiles. Each pixel belongs to the last
// tile starting at or before it and blends linearly with the raw tile above, then with the raw tile to the left,
// across their overlaps, the same order as the reference.
__global__ void unpatchify_blend_kernel(const __half* __restrict__ projected, TileGrid grid, float* __restrict__ canvas,
    ...
    for (size_t index = static_cast<size_t>(blockIdx.x) * blockDim.x + threadIdx.x; index < count; index += stride) {
        ...
        const int frame = static_cast<int>((index / (static_cast<size_t>(width) * height)) % frames);
        const int channel = static_cast<int>(index / (static_cast<size_t>(width) * height * frames));
        int tile_row = 0;
        ...
        if (tile_column > 0 && column < grid.overlaps_x[tile_column - 1]) {
            ...
            const float left = tile_pixel(projected, grid, tile - 1, channel, frame, row, grid.tile_width - extent + column);
...
// Writes `count` canvas frames starting at `first` into the output at `position`, blending the first
// `blend_frames` with the saved overlap, then maps them to pixels in [0, 1].
__global__ void write_frames_kernel(const float* __restrict__ canvas, int canvas_frames, int first, int count,
                                    const float* __restrict__ overlap, int blend_frames, float* __restrict__ output,
                                    int output_frames, int position, int plane, float mean_red, float mean_green,
                                    float mean_blue, float deviation_red, float deviation_green, float deviation_blue) {
    const size_t total = static_cast<size_t>(3) * count * plane;
    const size_t stride = static_cast<size_t>(gridDim.x) * blockDim.x;
    for (size_t index = static_cast<size_t>(blockIdx.x) * blockDim.x + threadIdx.x; index < total; index += stride) {
        ...
        float value = canvas[(static_cast<size_t>(channel) * canvas_frames + first + frame) * plane + pixel];
        if (frame < blend_frames) {
            const float weight = static_cast<float>(frame) / blend_frames;
            const float previous = overlap[(static_cast<size_t>(channel) * blend_frames + frame) * plane + pixel];
            value = previous * (1.0f - weight) + value * weight;
        }
        const float mean = channel == 0 ? mean_red : (channel == 1 ? mean_green : mean_blue);
        const float deviation = channel == 0 ? deviation_red : (channel == 1 ? deviation_green : deviation_blue);
        value = fminf(fmaxf(value * deviation + mean, 0.0f), 1.0f);
        output[(static_cast<size_t>(channel) * output_frames + position + frame) * plane + pixel] = value;
    }
}

// Saves `count` canvas frames starting at `first` as the overlap for the next chunk.
__global__ void save_overlap_kernel(const float* __restrict__ canvas, int canvas_frames, int first, int count,
                                    float* __restrict__ overlap, int plane) {
    const size_t total = static_cast<size_t>(3) * count * plane;
    const size_t stride = static_cast<size_t>(gridDim.x) * blockDim.x;
    for (size_t index = static_cast<size_t>(blockIdx.x) * blockDim.x + threadIdx.x; index < total; index += stride) {
        const int pixel = static_cast<int>(index % plane);
...
extern "C" int mmh3_vae_qk_norm_rope(__half* qkv, const float* angles, int pairs, int tile_tokens, int tokens,
                                     int heads, float epsilon, cudaStream_t stream) {
    ...
    qk_norm_rope_kernel<<<blocks, ROW_THREADS, 0, stream>>>(qkv, angles, pairs, tile_tokens, tokens, heads, epsilon);
    return static_cast<int>(cudaGetLastError());
}

extern "C" int mmh3_vae_residual_add_scaled(float* residual, const __half* delta, const __half* scale, int tokens,
                                            int width, cudaStream_t stream) {
    const size_t count = static_cast<size_t>(tokens) * width;
    residual_add_scaled_kernel<<<grid_for(count), ROW_THREADS, 0, stream>>>(residual, delta, scale, count, width);
    return static_cast<int>(cudaGetLastError());
}

extern "C" int mmh3_vae_swiglu(const __half* input, __half* output, int tokens, int width, cudaStream_t stream) {
    const size_t count = static_cast<size_t>(tokens) * width;
    swiglu_kernel<<<grid_for(count), ROW_THREADS, 0, stream>>>(input, output, count, width);
    return static_cast<int>(cudaGetLastError());
}

extern "C" int mmh3_vae_embed_suffix(const __half* embedded, const __half* registers, float* residual, int tiles,
                                     int tile_tokens, int patches, int registers_count, int width, cudaStream_t stream) {
    const size_t count = static_cast<size_t>(tiles) * tile_tokens * width;
    embed_suffix_kernel<<<grid_for(count), ROW_THREADS, 0, stream>>>(embedded, registers, residual, tile_tokens, patches,
                                                                     registers_count, count, width);
    return static_cast<int>(cudaGetLastError());
}

extern "C" int mmh3_vae_unpatchify_blend(const __half* projected, const int* starts_y, const int* starts_x,
                                         const int* overlaps_y, const int* overlaps_x, int tiles_y, int tiles_x,
                                         int tile_height, int tile_width, int latent_frames, int tile_tokens,
                                         float* canvas, int height, int width, cudaStream_t stream) {
    TileGrid grid = {starts_y, starts_x, overlaps_y, overlaps_x, tiles_y, tiles_x, tile_height, tile_width,
                     latent_frames, tile_tokens};
    const size_t count = static_cast<size_t>(3) * latent_frames * PATCH_FRAMES * height * width;
    unpatchify_blend_kernel<<<grid_for(count), ROW_THREADS, 0, stream>>>(projected, grid, canvas, height, width);
    return static_cast<int>(cudaGetLastError());
}

extern "C" int mmh3_vae_write_frames(const float* canvas, int canvas_frames, int first, int count, const float* overlap,
                                     int blend_frames, float* output, int output_frames, int position, int plane,
                                     const float* mean, const float* deviation, cudaStream_t stream) {
    const size_t total = static_cast<size_t>(3) * count * plane;
    write_frames_kernel<<<grid_for(total), ROW_THREADS, 0, stream>>>(
        canvas, canvas_frames, first, count, overlap, blend_frames, output, output_frames, position, plane, mean[0],
        mean[1], mean[2], deviation[0], deviation[1], deviation[2]);
    return static_cast<int>(cudaGetLastError());
}

extern "C" int mmh3_vae_save_overlap(const float* canvas, int canvas_frames, int first, int count, float* overlap,
