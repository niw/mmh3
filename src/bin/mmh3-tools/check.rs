...
use mmh3::models::{AUDIO_VAE_FILE, TEXT_ENCODER_FILE, VIDEO_VAE_FILE, load_dit, option_path, sparse_attention};
use mmh3_core::json;
...
fn compare(actual: &[f32], expected: &[f32]) -> (f64, f64, f32, f32) {
    ...
    let difference: f64 =
        actual.iter().zip(expected).map(|(&left, &right)| (left as f64 - right as f64).powi(2)).sum::<f64>().sqrt();
...
fn check_dit(arguments: &[String]) -> Result<(), Box<dyn Error>> {
    ...
    let options = parse_options(
        arguments,
        &["golden", "models", "weights", "lora", "lora-strength", "attention", "attention-precision", "sparse-tau"],
        USAGE,
    )?;
    ...
    let routing =
        outputs.routed_fraction.map_or(String::new(), |fraction| format!(", Sol-Attn routed {:.1}%", 100.0 * fraction));
    ...
    for (index, residual) in &outputs.blocks {
        ...
        for segment in ["text", "audio", "video"] {
            ...
            for (sample, (row, expected_row)) in
                sampled.chunks_exact(hidden).zip(expected.data.chunks_exact(hidden)).enumerate()
            {
...
fn check_sample(arguments: &[String]) -> Result<(), Box<dyn Error>> {
    ...
    let (shift_video, shift_audio): (f32, f32) =
        (dit_file.metadata("shift_video")?.parse()?, dit_file.metadata("shift_audio")?.parse()?);
    ...
    for step in 0..schedule.steps() {
        ...
        let (video_cosine, video_relative, _, _) =
            compare(&video.data, &dit_file.tensor(&format!("step{step}.latent.video"))?.data);
        let (audio_cosine, audio_relative, _, _) =
            compare(&audio.data, &dit_file.tensor(&format!("step{step}.latent.audio"))?.data);
        println!(
...
fn check_video_vae(arguments: &[String]) -> Result<(), Box<dyn Error>> {
    ...
    let psnr = |actual: &[f32], expected: &[f32]| {
        let squared: f64 =
            actual.iter().zip(expected).map(|(&left, &right)| (left as f64 - right as f64).powi(2)).sum();
...
fn check_audio_vae(arguments: &[String]) -> Result<(), Box<dyn Error>> {
    ...
    for (label, name) in [("ComfyUI default", "audio"), ("ComfyUI FP32", "audio_float32")] {
        ...
        if waveform.shape != expected.shape {
            return Err(
                format!("decoded shape {:?} differs from the golden {:?}", waveform.shape, expected.shape).into()
            );
        }
...
fn check_text_encoder(arguments: &[String]) -> Result<(), Box<dyn Error>> {
    ...
    let expected_ids: Vec<u32> =
        golden.0.data(id_info).as_chunks::<8>().0.iter().map(|&bytes| i64::from_le_bytes(bytes) as u32).collect();
    ...
    let captured: Vec<usize> =
        golden.0.tensors().iter().filter_map(|info| info.name.strip_prefix("layer.")?.parse().ok()).collect();
    let started = Instant::now();
