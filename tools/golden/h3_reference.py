"""Writes golden data for a text-to-video generation with the released weights through ComfyUI.

Development-only tool. Run it with the Python environment of a ComfyUI checkout that supports MiniMax H3 and has
the released checkpoints in its models directory, on a GPU, for example:

    python3 tools/golden/h3_reference.py --comfyui /path/to/ComfyUI --out /path/to/golden/small

The defaults describe the small case (448x256, 22 frames). The 768p case is
--width 1344 --height 768 --frames 124 --block-row-stride 16 --no-decode. --compute-dtype float32 runs the DiT with an
FP32 residual stream and FP32 activations instead of ComfyUI's default BF16, as a tighter reference. --lora applies a
LoRA from models/loras to the DiT, for example the 768p Turbo LoRA with --steps 4 --shift-video 6. ComfyUI merges a
LoRA into quantized weights and requantizes them with stochastic rounding, and --lora-rounding nearest rounds to
nearest instead. --sparse-attention sol patches in ComfyUI's Sol-Attn from the step at --sparse-start of the run.

It writes up to three files into the output directory:

- text.safetensors: prompt token ids, the text encoder hidden states and the per-token modality tags. An existing
  file is reused, so copying it from another case skips the text encoder.
- dit.safetensors: initial noise, the latents after every step of a plain Euler sampler on the unshifted grid
  linspace(1, 0, steps + 1), and the prediction and selected block outputs of one step. The stored velocity is the
  data-ward prediction u with x0 = x + sigma · u, which is the negated flow velocity ComfyUI's model returns. Step 0
  runs at sigma 1 where the video and audio timesteps coincide, so step 1 is captured by default.
...
def temporal_shape(frames):
    """Frame count snapped to the 17k + 5 grid, the latent frame count and the audio latent frame count."""
    while frames % 17 != 5:
        frames += 1
...
def main():
    ...
    parser.add_argument("--prompt", default="A red panda sips tea on a sunny wooden porch while birds chirp in the garden.")
    ...
    parser.add_argument("--lora-rounding", choices=["stochastic", "nearest"], default="stochastic",
    ...
    parser.add_argument("--sparse-start", type=float, default=0.2,
    ...
    metadata = {key: str(value) for key, value in vars(arguments).items() if key not in ("comfyui", "out")}

    if arguments.decode_only:
        from safetensors import safe_open

        with safe_open(os.path.join(arguments.out, "dit.safetensors"), framework="pt") as file:
    ...
    if os.path.exists(text_path):
        # Text encoding takes minutes, so an existing result for the same prompt is reused.
        saved = load_file(text_path)
        context, tags = saved["context"][None], saved["token_tags"]
        print(f"text: reused {text_path}", flush=True)
    else:
        clip = comfy.sd.load_clip(ckpt_paths=[os.path.join(models, TEXT_ENCODER)], clip_type=comfy.sd.CLIPType.MINIMAX)
        ...
        }, text_path, metadata=metadata)
        print(f"text: {len(token_ids)} tokens, context {tuple(context.shape)}, {time.time() - started:.1f} s", flush=True)
        del clip
    ...
    if arguments.lora:
        if arguments.lora_rounding == "nearest":
            # ComfyUI seeds stochastic rounding from each weight's key, and seed 0 means round to nearest.
            comfy.utils.string_to_seed = lambda key: 0
        lora = comfy.utils.load_torch_file(os.path.join(models, "loras", arguments.lora), safe_load=True)
        patcher, _ = comfy.sd.load_lora_for_models(patcher, None, lora, arguments.lora_strength, 0)
    if arguments.sparse_attention == "sol":
        from comfy_extras.nodes_sparse_attention import apply_block_sparse_attention

        # The window opens at sigma 1, and the loop below keeps the early steps dense by hiding their sigma.
        patcher = apply_block_sparse_attention(
            patcher, tau=arguments.sparse_tau, topk_ratio=0.0, vsa=False, start_percent=0.0, end_percent=1.0,
            min_tokens=12288, dense_blocks=set(), sink_conditioning="exact_kv_and_rows",
            extra_tokens=arguments.sparse_extra_tokens, verbose=False)
    ...
    compute_dtype = torch.float32 if arguments.compute_dtype == "float32" else patcher.model.get_dtype_inference()
    context = context.to(device=device, dtype=compute_dtype)
    frames, latent_frames, audio_frames = temporal_shape(arguments.frames)
    generator = torch.Generator().manual_seed(arguments.seed)
    video = torch.randn((1, 24, latent_frames, arguments.height // 16, arguments.width // 16), generator=generator)
    ...
    handles = [model.blocks[index].register_forward_hook(capture(index)) for index in watched]
    ...
    options = {"minimax_h3_sigma_shift_video": arguments.shift_video,
    ...
    with torch.inference_mode():
        for step in range(arguments.steps):
            ...
            if arguments.sparse_attention == "sol":
                ...
                step_options["sigmas"] = torch.tensor([sigmas_video[step] if sparse else 2.0])
            outputs = model._forward([video, audio], timestep, context, transformer_options=step_options,
                                     minimax_payload=payload)
            velocity_video, velocity_audio = (-outputs[0]).float(), (-outputs[1]).float()
            if capturing[0]:
            ...
            print(f"dit: step {step} at sigma {sigmas_video[step]:.4f} done, {time.time() - started:.1f} s", flush=True)
    ...
    for handle in handles:
        handle.remove()
    save_file({name: value.contiguous() for name, value in tensors.items()}, os.path.join(arguments.out, "dit.safetensors"),
              metadata=metadata)
    del patcher, model
    model_management.unload_all_models()
    model_management.soft_empty_cache()
...
def decode(arguments, models, device, video, audio, metadata):
    ...
    with torch.inference_mode():
        ...
        for name, dtype, tf32 in (("audio", None, allow_tf32), ("audio_float32", torch.float32, False)):
            ...
            waveform = audio_vae.first_stage_model.decode(audio.to(device=device, dtype=audio_vae.vae_dtype))
            decoded[name] = waveform.float().cpu().contiguous()
            print(f"decode: {name} with the audio VAE in {audio_vae.vae_dtype}, TF32 convolutions {tf32}", flush=True)
            del audio_vae
            model_management.unload_all_models()
        torch.backends.cudnn.allow_tf32 = allow_tf32
    save_file(decoded, os.path.join(arguments.out, "decode.safetensors"), metadata=metadata)
    print(f"decode: video {tuple(decoded['video'].shape)}, audio {tuple(decoded['audio'].shape)}, "
