...
// The activation side of INT8 ConvRot layers, shared by the DiT and VAE kernels: rotating 256-column groups by the
// normalized regular Hadamard matrix and quantizing rows to INT8.
...
// rounding half to even. The block's warps share the row, which may lie in global or shared memory: warp w rotates
// groups w, w + warps and so on.
template <int GROUPS_PER_WARP, typename Pair>
__device__ __forceinline__ void rotate_quantize_row(const Pair* row, int8_t* output_row, float* scale_output,
                                                    int columns) {
...
#pragma unroll
    for (int slot = 0; slot < GROUPS_PER_WARP; slot++) {
        ...
        if (group < groups) {
            const uint4 packed = *reinterpret_cast<const uint4*>(row + (group * CONVROT_GROUP + lane * 8) / 2);
...
#pragma unroll
            ...
            *reinterpret_cast<uint2*>(output_row + group * CONVROT_GROUP + lane * 8) = make_uint2(words[0], words[1]);
        }
    }
}
