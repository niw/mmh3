"""Measures ComfyUI's MiniMax H3 speed at 768p for 5 seconds as the baseline mmh3 has to beat.

Development-only tool. Run it with the Python environment of a ComfyUI checkout that supports MiniMax H3 and has
the checkpoints in its models directory, on a GPU with nothing else running, for example:

    python3 tools/golden/comfyui_benchmark.py --comfyui /path/to/ComfyUI

It times one DiT call with ComfyUI's default dense attention and with the Sol-Attn patch of the Model Sparse
Attention node at its defaults, and one video VAE decode, all on random inputs of the target shape.
"""
...
def main():
    ...
    audio = torch.randn((1, 32, 2, arguments.audio_frames), generator=generator).to(device)
    context = torch.randn((1, arguments.text_tokens, 5120), generator=generator).to(device=device, dtype=dtype)
    payload = {"text_token_tags": torch.ones(arguments.text_tokens, dtype=torch.int64)}
    timestep = torch.tensor([500.0], device=device)
    tokens = arguments.text_tokens + 2 * arguments.audio_frames + shape[0] * shape[1] * shape[2] // 4
    results["tokens"] = tokens

    with torch.inference_mode():
        results["dit_dense_seconds"] = timed(
            lambda: model._forward([video, audio], timestep, context, transformer_options={}, minimax_payload=payload),
            arguments.runs)
        print(json.dumps(results), flush=True)

        sparse = apply_block_sparse_attention(
            patcher, tau=1.3, topk_ratio=0.0, vsa=False, start_percent=0.0, end_percent=1.0, min_tokens=12288,
            dense_blocks=set(), sink_conditioning="exact_kv_and_rows", extra_tokens=256, verbose=False)
        options = sparse.model_options["transformer_options"]
        results["dit_sol_attn_seconds"] = timed(
            lambda: model._forward([video, audio], timestep, context, transformer_options=dict(options),
                                   minimax_payload=payload),
            arguments.runs)
        print(json.dumps(results), flush=True)
    del sparse, patcher, model
    model_management.unload_all_models()
    model_management.soft_empty_cache()

    with torch.inference_mode():
        vae = comfy.sd.VAE(sd=comfy.utils.load_torch_file(os.path.join(models, VIDEO_VAE)))
        model_management.load_models_gpu([vae.patcher], force_full_load=True)
        latent = torch.randn((1, 24) + shape, generator=generator).to(device=device, dtype=vae.vae_dtype)
        results["video_vae_decode_seconds"] = timed(lambda: vae.first_stage_model.decode(latent), 1)
    print(json.dumps(results), flush=True)
