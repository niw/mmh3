//! Compares the reference DiT with golden data from ComfyUI's implementation (tools/golden/dit_tiny.py).
...
fn tensor(file: &SafeTensors, name: &str) -> Tensor {
    Tensor::load(file, file.get(name).unwrap_or_else(|| panic!("missing {name}"))).unwrap()
}
...
fn assert_close(name: &str, actual: &[f32], expected: &[f32]) {
    ...
    assert!(cosine > 0.999_999 && worst <= scale * 1e-4, "{name}: cosine {cosine}, max error {worst}, scale {scale}");
}
...
fn matches_comfyui_golden_forward() {
    ...
    let config = DitConfig::from_shapes(|name| tensors.get(name).map(|tensor| tensor.shape.clone())).unwrap();
    assert_eq!((config.hidden, config.heads, config.layers, config.refiner_layers), (192, 2, 2, 1));
    ...
    assert_close("text states", &trace.text_states, &tensor(&file, "intermediate.text_states").data);
    for (index, output) in trace.block_outputs.iter().enumerate() {
        assert_close(&format!("block {index}"), output, &tensor(&file, &format!("intermediate.block.{index}")).data);
    }
