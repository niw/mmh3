...
fn main() {
    ...
    let arch = match env::var("CARGO_CFG_TARGET_ARCH").unwrap().as_str() {
        "aarch64" => "arm64",
        "x86_64" => "x86_64",
        other => panic!("unsupported VideoToolbox target architecture {other}"),
    };
    ...
    assert!(
        Command::new("xcrun")
            ...
            .success(),
        "compiling the Swift VideoToolbox runtime failed"
    );
    assert!(
        Command::new("xcrun")
            .args(["ar", "rcs"])
            .arg(out.join("libmmh3_videotoolbox.a"))
            .arg(object)
            .status()
            .unwrap()
            .success()
    ...
    println!("cargo:rustc-link-lib=static=mmh3_videotoolbox");
