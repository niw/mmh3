"""Writes golden data for a tiny MiniMax H3 video VAE decoder from ComfyUI's code.

Development-only tool. Run it with the Python environment of a ComfyUI checkout that
supports MiniMax H3, like dit_tiny.py:

    python3 tools/golden/vae_tiny.py --comfyui /path/to/ComfyUI \\
        --out tests/fixtures/vae_tiny.safetensors

ComfyUI's MiniMaxH3VideoVAE gets a random two-layer ViT decoder and 32-pixel tiles, so a
12-frame latent of 4 × 6 decodes as 15 spatial tiles in two temporal chunks and
exercises every blend. The file holds the weights with the checkpoint's names in F16,
the input latent, the raw decoder output of the first tile and the final pixels in [0,
1], all computed in FP32 on the CPU.
"""
...
def main():
    ...
    model = h3_vae.MiniMaxH3VideoVAE(
        tile_size=TILE_SIZE, tile_overlap_min=TILE_OVERLAP_MIN
    )
    model.decoder = h3_vae.ViT3DDecoder(
        **DECODER, operations=comfy.ops.disable_weight_init
    )
    ...
    parameters.update(
        dict(model.post_quant_conv.named_parameters(prefix="post_quant_conv"))
    )
    for name, tensor in parameters.items():
        if name.endswith(("norm1.weight", "norm2.weight", "norm_out.weight")):
            value = 1.0 + 0.1 * torch.randn(tensor.shape, generator=generator)
        elif name.endswith(("scale1", "scale2")):
            value = 0.5 + 0.1 * torch.randn(tensor.shape, generator=generator)
        elif name.endswith((".bias", "register_tokens", "mask_token")):
    ...
    for name in ("latents_mean", "latents_std"):
        base = (
            torch.randn(24, generator=generator) * 0.5
            if name == "latents_mean"
            else 1.0 + torch.rand(24, generator=generator)
        )
    ...
    metadata = {
        ...
        "tile_overlap_min": str(TILE_OVERLAP_MIN),
        "decoder": json.dumps(DECODER),
    }
    save_file(tensors, arguments.out, metadata=metadata)
    print(
        f"wrote {len(tensors)} tensors, pixels {tuple(pixels.shape)}, first tile {tuple(captured[0].shape)}"
    )
