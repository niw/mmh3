...
template <>
struct MmaForm<MMA_KIND_BF16_F32> {
...
template <>
struct MmaForm<MMA_KIND_F16_F16> {
    static constexpr int operations = 2 * 16 * 8 * 16;
...
template <>
struct MmaForm<MMA_KIND_S8_S32> {
...
template <>
struct MmaForm<MMA_KIND_E4M3_F32> {
    static constexpr int operations = 2 * 16 * 8 * 32;
    using Accumulator = float[4];
    __device__ static void run(float (&c)[4], const uint32_t (&a)[4], const uint32_t (&b)[2]) {
        asm volatile(
...
template <>
struct MmaForm<MMA_KIND_E4M3_F16> {
    static constexpr int operations = 2 * 16 * 8 * 32;
    using Accumulator = uint32_t[2];
    __device__ static void run(uint32_t (&c)[2], const uint32_t (&a)[4], const uint32_t (&b)[2]) {
        asm volatile(
...
template <int KIND>
__global__ void __launch_bounds__(THREADS_PER_BLOCK) mma_peak_kernel(int iterations, uint32_t* sink) {
    using Form = MmaForm<KIND>;
    // Small positive operands in every format keep the loop free of special values.
    const uint32_t operand = KIND == MMA_KIND_S8_S32 ? 0x01010101u : (KIND >= MMA_KIND_E4M3_F32 ? 0x20202020u : 0x3C003C00u);
...
#pragma unroll
    ...
    uint32_t checksum = 0;
    for (int chain = 0; chain < CHAINS; chain++) {
        for (int element = 0; element < static_cast<int>(sizeof(accumulators[chain]) / 4); element++) {
...
template <int KIND>
cudaError_t launch_peak(int blocks, int iterations, uint32_t* sink) {
    mma_peak_kernel<KIND><<<blocks, THREADS_PER_BLOCK>>>(iterations, sink);
    return cudaGetLastError();
}

cudaError_t launch_kind(int kind, int blocks, int iterations, uint32_t* sink, int* operations_per_mma) {
    switch (kind) {
...
extern "C" int mmh3_bench_mma_peak(int kind, int iterations, float* tera_operations_per_second, char* message,
                                   size_t message_size) {
