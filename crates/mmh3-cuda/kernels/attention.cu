...
// Dense attention in the FlashAttention-2 style for head dimensions 64 and 128, BF16 or FP16, bidirectional or
// causal, with grouped key and value heads. The strided layout covers the DiT's fused [q | k | v] projection, the VAE
// decoder's per-head [q, k, v] layout and the text encoder's [q | k | v] with fewer key and value heads alike.
...
static_assert(BLOCK_M * Tiles<64>::row_bytes <= Tiles<64>::stage_bytes, "the query tile must fit in one stage");
static_assert(BLOCK_M * Tiles<128>::row_bytes <= Tiles<128>::stage_bytes, "the query tile must fit in one stage");
...
__global__ void __launch_bounds__(THREADS, 1)
    ...
    auto head_base = [&](const Element* base, Operand operand) {
        ...
        return base + batch * layout.batch_stride[operand] + operand_head * layout.head_stride[operand];
    };
    ...
    auto load_key_value_block = [&](int block, int stage) {
        load_rows<HEAD_DIM, THREADS>(stage_address(stage), key_head, layout.token_stride[KEY], block * BLOCK_N, BLOCK_N, tokens);
        load_rows<HEAD_DIM, THREADS>(stage_address(stage) + T::tile_bytes, value_head, layout.token_stride[VALUE],
                            block * BLOCK_N, BLOCK_N, tokens);
    };

    // The query tile borrows stage 1 until it has been copied into registers.
    load_rows<HEAD_DIM, THREADS>(stage_address(1), query_head, layout.token_stride[QUERY], first_query, BLOCK_M, tokens);
    copy_async_commit();
...
#pragma unroll
        ...
        // NOTE: every query row sees key 0 in the first block, so a causal row never starts from an all-masked block.
        const bool masked_block =
            (block + 1) * BLOCK_N > tokens || (layout.causal && (block + 1) * BLOCK_N - 1 > first_query);
...
#pragma unroll
            for (int element = 0; element < 4; element++) {
                ...
                if (masked_block) {
                    const int key_index = block * BLOCK_N + key_tile_index * 8 + (lane % 4) * 2 + (element % 2);
...
#pragma unroll
    ...
    Element* output_head = output + batch * layout.batch_stride[OUTPUT] + head * layout.head_stride[OUTPUT];
#pragma unroll
    for (int half = 0; half < 2; half++) {
        ...
        Element* output_row = output_head + static_cast<int64_t>(token) * layout.token_stride[OUTPUT];
#pragma unroll
        for (int dimension_tile = 0; dimension_tile < HEAD_DIM / 8; dimension_tile++) {
...
template <typename Element, int HEAD_DIM>
int launch(const void* query, const void* key, const void* value, void* output, int tokens, int heads, int batch,
    ...
    normalized.heads_per_key_value = layout.heads_per_key_value > 0 ? layout.heads_per_key_value : 1;
    dim3 grid((tokens + BLOCK_M - 1) / BLOCK_M, heads, batch);
    kernel<<<grid, THREADS, Tiles<HEAD_DIM>::shared_bytes, stream>>>(
        static_cast<const Element*>(query), static_cast<const Element*>(key), static_cast<const Element*>(value),
        static_cast<Element*>(output), tokens, normalized, scale * 1.4426950408889634f);
...
extern "C" int mmh3_attention(int element_type, int head_dim, const void* query, const void* key, const void* value,
                              void* output, int tokens, int heads, int batch, const Mmh3AttentionLayout* layout,
                              float scale, cudaStream_t stream) {
    if (tokens <= 0 || heads <= 0 || batch <= 0 || (layout->heads_per_key_value > 0 && heads % layout->heads_per_key_value != 0)) {
        return static_cast<int>(cudaErrorInvalidValue);
    }
    if (element_type == 0 && head_dim == 128) {
        return launch<__nv_bfloat16, 128>(query, key, value, output, tokens, heads, batch, *layout, scale, stream);
    }
    if (element_type == 1 && head_dim == 64) {
        return launch<__half, 64>(query, key, value, output, tokens, heads, batch, *layout, scale, stream);
    }
    if (element_type == 0 && head_dim == 64) {
        return launch<__nv_bfloat16, 64>(query, key, value, output, tokens, heads, batch, *layout, scale, stream);
    }
    if (element_type == 1 && head_dim == 128) {
        return launch<__half, 128>(query, key, value, output, tokens, heads, batch, *layout, scale, stream);
    }
    return static_cast<int>(cudaErrorInvalidValue);
}
...
extern "C" int mmh3_attention_bf16(const __nv_bfloat16* query, const __nv_bfloat16* key, const __nv_bfloat16* value,
                                   ...
                                   cudaStream_t stream) {
    Mmh3AttentionLayout layout = {{query_stride, key_stride, value_stride, output_stride}, {128, 128, 128, 128}, {0, 0, 0, 0}, 1, 0};
    return mmh3_attention(0, 128, query, key, value, output, tokens, heads, 1, &layout, scale, stream);
}
